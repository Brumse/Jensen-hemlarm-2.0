...
char* build_json_payload(const char* device_id, const char* key, const char* value) {
...
CURLcode send_http_post(const char* url, const char* payload, struct MemoryStruct *chunk) {
    CURL *curl;
...
void register_device() {
    ...
    snprintf(register_url, sizeof(register_url), "%s/register_device", API_BASE_URL);
    char* payload = build_json_payload(DEVICE_ID, "name", DEVICE_NAME);
    ...
    if (res == CURLE_OK) {
        ...
        printf("Device registration successful. Status code: %ld, Response: %s\n", response_code, chunk.memory);
    } else {
...
void on_message(struct mosquitto *mosq, void *userdata, const struct mosquitto_message *msg) {
    if (msg->payloadlen > 0) {
        ...
        printf("Device: %s, Alarm Active: %d, Distance: %.2f\n", device_str, alarm_int, distance_val);

        char motion_url[256];
        snprintf(motion_url, sizeof(motion_url), "%s/motion_detected", API_BASE_URL);
        ...
        json_object_set_new(motion_data, "alarm_active", json_boolean(alarm_int)); // alarm_active
        ...
        json_decref(root);
	...
	else if (strcmp(msg->topic, "/alarm/status") == 0) {
            // handle willmessage
            json_error_t error;
            ...
            printf("Will Message Received - Device ID: %s, Name: %s, Status: %s\n", device_id, name, status);

            
            char api_url[256];
            snprintf(api_url, sizeof(api_url), "%s/device_status", API_BASE_URL); 
...
int main() {
    ...
    if (mosquitto_connect(mosq, MQTT_BROKER_IP, MQTT_PORT, 60) != MOSQ_ERR_SUCCESS) {
