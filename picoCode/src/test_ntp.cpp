...
static void ntp_result(NTP_T *state, int status, time_t *result) {
    if (status == 0 && result) {
        ...
        printf("got ntp response: %02d/%02d/%04d %02d:%02d:%02d\n",
               ...
               local_tm->tm_year + 1900, local_tm->tm_hour, local_tm->tm_min,
               local_tm->tm_sec);
    }
...
void run_ntp_test(void) {
    ...
    if (!state)
        return;
    while (true) {
        if (absolute_time_diff_us(get_absolute_time(), state->ntp_test_time) <
                0 &&
        ...
        cyw43_arch_wait_for_work_until(state->dns_request_sent
                                           ? at_the_end_of_time
                                           : state->ntp_test_time);
#else
